...
class PDFLabel(FPDF):
    ...
    def __init__(self, format, unit="mm", posX=1, posY=1):
        ...
        self.margin_left = self.convert_metric(
            type_format["marginLeft"], type_format["metric"]
        )  # Left margin of labels
        self.margin_top = self.convert_metric(
            type_format["marginTop"], type_format["metric"]
        )  # Top margin of labels
        self.space_x = self.convert_metric(
            type_format["SpaceX"], type_format["metric"]
        )  # Horizontal space between 2 labels
        self.space_y = self.convert_metric(
            type_format["SpaceY"], type_format["metric"]
        ...
        self.width = self.convert_metric(
            type_format["width"], type_format["metric"]
        )  # Width of label
        self.height = self.convert_metric(
            type_format["height"], type_format["metric"]
        )  # Height of label
        self.line_height = self.get_height_chars(
            type_format["font-size"]
        )  # Line height
    ...
    def add_label(self, text, qrcode):
        """Print a label"""
        self.countX += 1
        ...
        pos_x = (
            self.margin_left + self.countX * (self.width + self.space_x) + self.padding
        )
        pos_y = (
            self.margin_top + self.countY * (self.height + self.space_y) + self.padding
        )
        width = self.width
        ...
        self.image(
            qrcode,
            (pos_x + (width - (padding + qr_size + 5))),
            pos_y,
            qr_size,
            qr_size,
            "PNG",
        )
        self.multi_cell(width, height, text, 0, "L")
